...
class ChooseBackGroundColorFragment : BaseFragment<FragmentColorBackgroundBinding>() {
    ...
    private var _isProcessingClick: Boolean = false
    ...
    private fun handleSingleClick(action: () -> Unit) {
        if (!_isProcessingClick) {
            ...
            action()
            // Đặt lại cờ sau một khoảng thời gian ngắn để ngăn chặn click nhanh chóng
            Handler().postDelayed({ _isProcessingClick = false }, 300)
        }
    }
